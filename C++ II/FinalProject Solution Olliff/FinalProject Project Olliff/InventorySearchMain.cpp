...
int main()
{
	...
	StoredOrders storage("processedOrders.dat");
	...
	while (!done)
	{
		...
		case 1:
			...
			if (searchResult != -1)
			{
				...
				storage.saveOneRecord(itemFound, quantity, priceFound*quantity);
			}
			else
			{
		...
		case 2:
			...
			orderTotal = storage.calcTotalCostOfOrders();
			storage.displayAllOrders();
			cout << setprecision(2) << fixed;
			cout << endl << endl;
			cout << "Total cost of all Orders: " << orderTotal << endl << endl;
